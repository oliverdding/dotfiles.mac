...
sudo port install -bcqN bash bash-completion bat bottom curl  exa fd fzf git git-delta gnupg2 less neovim pass ripgrep starship xplr zoxide zstd
...
sudo port install -bcqN alacritty
